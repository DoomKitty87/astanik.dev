...
float map(vec3 p) {
  vec3 q = p - 0.75 * round(p / 0.75);
  q.y = p.y;
  
  float d = sdRoundBox(q, vec3(0.25), 0.1, p.xz * 0.1 + u_time * 0.05, 1.);
...
vec3 noiseColor(vec3 p) {
  p *= 0.5;
  return vec3(0.5) + 0.5 * sin(vec3(0.1, 0.2, 0.3) * p + vec3(0.0, 2.0, 4.0) + u_time * 0.2);
}
